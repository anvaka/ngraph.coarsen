...
test('simple graph with node as community', function(t) {
  ...
  t.equal(newGraph.getLinksCount(), 1, 'There is exactly one link');
  t.equal(newGraph.getNodesCount(), 2, 'There are two nodes');

  var link = newGraph.getLink(1, 2);
  t.ok(link, 'Link exists')
  t.equal(link.data, 1, 'Weight is correct.')
...
test('it adds all nodes inside a community', function(t) {
  ...
  t.equal(community.data.size, 2, 'Both nodes are added');
...
test('it accumulates weights', function(t) {
  ...
  t.equal(newGraph.getLinksCount(), 3, 'There should be three links');
  t.equal(newGraph.getNodesCount(), 2, 'There are two nodes: even and odd');

  var loop = newGraph.getLink(1, 1);
  t.ok(loop, 'loop link exists')
  t.equal(loop.data, 1, 'loop weight is correct.')

  var link01 = newGraph.getLink(0, 1);
  t.ok(link01, '0 -> 1 link exists')
  t.equal(link01.data, 1, 'Weight is correct.')

  var link10 = newGraph.getLink(1, 0);
  t.ok(link10, '1 -> 0 link exists')
  t.equal(link10.data, 1, 'Weight is correct.')
...
test('it can handle isolate nodes', function(t) {
  ...
  t.equal(newGraph.getLinksCount(), 1, 'There should be one link');
...
test('it adds srcGraph to response', function(t) {
  ...
  var newGraph = coarsen(srcGraph, fakeCommunity);
  t.equal(newGraph.srcGraph, srcGraph, 'srcGraph is here.');
...
test('it can get all subgraphs', function(t) {
  ...
  t.equal(allGraphs.length, 2, 'Two communities - two graphs');
  allGraphs.forEach(function(record) {
    var node = coarseGraph.getNode(record.id);
    t.ok(node, 'Coarse node id is valid');
    t.equal(node.data.size,  record.graph.getNodesCount(), 'Number of nodes in subgraph is valid');
  });
